...
async function handleEvent(event) {
  ...
  // location
  else if (userMessage.match(/位置|地址|導航|location/)) {
    reply = {
      type: "location",
      title: "金品早午餐",
  ...
  // phone
  else if (userMessage.match(/電話|聯絡|訂餐/)) {
    reply = { type: "text", text: "📞 聯絡電話：05-2060286" };
  } 
  // open time
  else if (userMessage.match(/營業|開幾點|關|休息/)) {
    reply = {
      type: "flex",
      altText: "營業時間資訊",
      contents: {
  ...
  // default
    ...
    reply = { type: "text", text: "謝謝光臨金品早餐 ☀️" };
  }

  return client.replyMessage(event.replyToken, reply).catch(err => {
